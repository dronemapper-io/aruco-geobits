...
namespace {
	...
	const char* keys =
		...
		"{i0       |       | 1st Input JPG image }"
		"{i1       |       | 2nd Input JPG image }"
		"{i2       |       | 3rd Input JPG image }"
		"{i3       |       | 4th Input JPG image }"
...
int main(int argc, char *argv[]) {
	...
	string imageName0 = parser.get<string>("i0");
	string imageName1 = parser.get<string>("i1");
	string imageName2 = parser.get<string>("i2");
	string imageName3 = parser.get<string>("i3");
	...
	//start loop on input images
	for (int i = 0; i < 4; i++) {
		cout << i << "\n";

		Mat image, imageCopy;
		if (i == 0) {
			image = imread(imageName0.c_str(), IMREAD_COLOR);
			cout << "DrnMppr: Processing -> " << imageName0 << "\n";
		}
		if (i == 1) {
			image = imread(imageName1.c_str(), IMREAD_COLOR);
			cout << "DrnMppr: Processing -> " << imageName1 << "\n";
		}
		if (i == 2) {
			image = imread(imageName2.c_str(), IMREAD_COLOR);
			cout << "DrnMppr: Processing -> " << imageName2 << "\n";
		}
		if (i == 3) {
			image = imread(imageName3.c_str(), IMREAD_COLOR);
			cout << "DrnMppr: Processing -> " << imageName3 << "\n";
		}
		...
		std::ostringstream detectImg;
		detectImg << "detect_";
		detectImg << i;
		detectImg << ".jpg";
		cout << "DrnMppr: Write -> " << detectImg.str() << "\n";

		imwrite(detectImg.str(), imageCopy);
		if (ids.size() > 0) {
	...
	if (allCharucoCorners.size() < 4) {
		cout << "Warning found " << allCharucoCorners.size() << " corners: Need 4\n";
		//return 0;
	}
